...
def functional(filepath, name=None, plot=False):
    ...
    ----------
    filepath: str
        path to <PROJECT_NAME>_job,log file
    name: str
        name of the project. If not given, will be inferred from filename. Default: None
    plot: bool
        if true will produce a plot of the functional values vs iterations. Default: False
    Returns
    -------
    iter_all: numpy.array
        array of iteration numbers of the run of size (number_of_iterations, 1)
    func: numpy.array
        array of functional of size (number_of_iterations, 1)
    """
...
def steplen(filepath, name=None, plot=False):
    ...
    name: str
        name of the project. If not given, will be inferred from filename. Default: None
    plot: bool
        if true will produce a plot of the step length values vs iterations. And highlight
    ...
    iter_all: numpy.array
        array of iteration numbers of the run of size (number_of_iterations, 1)
    func: numpy.array
        array of step length values of size (number_of_iterations, 1)
    """
